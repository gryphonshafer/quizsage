...
export default class Material {
    ...
    static text2words (text) {
        return text
            ...
            // remove sigle-quote following a non-word character
            .replaceAll( /(?<!\w)'/g, ' ')
            // remove sigle-quote following a word character prior to a non-word
            .replaceAll( /(\w)'(?=\W|$)/g, '$1')
    ...
    search ( text, exact = false, bible = undefined ) {
        return ( ( ! bible ) ? this.all_verses : this.verses_by_bible[bible] )
            .filter( verse => verse.text.match(
                ( ! exact )
                    ? new RegExp( Material.text2words(text).join('\\W+'), 'i' )
                    : new RegExp( text.split(/\s/).join('\\s+') )
            ) )
            .sort( ( a, b ) =>
                a.book    - b.book    ||
                a.chapter - b.chapter ||
